...
- [x] **Configurado + testado na AWS** ([D-19]): bucket `adimplo-db-backups-completo-cassio` (us-east-2), IAM user `adimplo-backup` com policy escopada só ao bucket, chaves no `.env` do EC2. Backup enviado e confirmado (`prod/postgres/*.sql.gz`).
- [x] **Lifecycle rule** no bucket (`expira-backups-90d`, prefixo `prod/`): expira backups remotos > 90 dias — resolve o "se lotar" remoto.
- [x] **Restore COMPROVADO**: dump restaurado num banco descartável (`restore_test`) no EC2, contagem batendo com produção (`Account` = 3 em ambos). Backup não é mais "torcida" — é testado.
- [ ] **(futuro) Migrar para Cloudflare R2 na produção** (barato + egress grátis) — só trocar `BACKUP_S3_ENDPOINT` + credenciais. 🔵
