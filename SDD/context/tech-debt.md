...
### D-24 · Cobrança do SaaS: gateway real da plataforma + renovação automática (spec 0020)
- **O quê**: a cobrança do próprio SaaS (planos/trial/gating) está implementada e testada, mas com **gateway de PLATAFORMA em mock** (`PLATFORM_PAYMENT_PROVIDER` default `mock`). Falta: (a) conta/credenciais REAIS da Adimplo num gateway (definir `PLATFORM_PAYMENT_PROVIDER` + segredos em env); (b) **cobrança automática de renovação** — hoje o cron `POST /system/platform-billing/run` só marca `past_due` quando o período vence; gerar a `PlatformInvoice` de renovação + cobrar automaticamente ficou de fora da v1; (c) quota não é aplicada na geração recorrente (`createForSubscription`) para não quebrar o sweep.
- **Impacto**: dev/demo 100% ok (checkout+webhook via mock ativam a assinatura). Para faturar de verdade os tenants, resolver (a) e (b).
- **Ação**: plugar o gateway real da plataforma; no `runRenewals`, gerar+cobrar a renovação (reusar `checkout`/`PlatformInvoice`) antes de marcar `past_due`; decidir quota na recorrência. Ver `SDD/specs/0020-cobranca-saas-planos.md`.
